...
class QuizApp:
    ...
    def create_widgets(self):
        ...
        for i in range(num_questions):
            ...
            check_question_button = Button(
                self.main_frame,
                text="Check",
                command=lambda index=i, entry=user_answer: self.check_answer(index, entry)
            )
            check_question_button.grid(
                row=i+1,
                column=2,
                padx=5,
                pady=5
            )
            self.question_widgets.append(check_question_button)
    ...
    def check_answer(self, index, entry):
        ...
        correct = self.quiz.check_answer_at(index, user_answer)
